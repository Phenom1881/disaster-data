...
def get_year_events(year, filenames):
    year_text = str(year)
    if year_text in _year_cache:
        return _year_cache[year_text]
    if year_text not in filenames:
        print(f"No NCEI file published yet for {year_text}.", file=sys.stderr)
        _year_cache[year_text] = pd.DataFrame()
        return _year_cache[year_text]
    _year_cache[year_text] = download_year_details(filenames[year_text])
    return _year_cache[year_text]
...
def events_near_date(state, center_date, window_days, filenames):
    ...
    frames = [get_year_events(year, filenames) for year in range(start.year, end.year + 1)]
    frames = [frame for frame in frames if not frame.empty]
    if not frames:
        return pd.DataFrame()
    events = pd.concat(frames, ignore_index=True)
    mask = (
        (events["STATE"].fillna("").str.upper() == state.upper())
...
def main():
    ...
    for _, declaration in declarations.iterrows():
        ...
        if matches.empty:
            print(
                f"{declaration['eo_number']}: no matching storm events found "
                "in the date window."
            )
            continue

        matches = matches.copy()
        matches["eo_number"] = declaration["eo_number"]
        matches["event_description"] = declaration["event_description"]
        matches["date_signed"] = declaration["date_signed"]
        matches["area_type"] = matches["CZ_TYPE"].map(AREA_TYPE_LABELS).fillna(
            "unknown NCEI area type"
        )
        all_matches.append(matches.reindex(columns=OUTPUT_COLUMNS))
    ...
    result = pd.concat(all_matches, ignore_index=True).sort_values(
        ["eo_number", "CZ_TYPE", "CZ_NAME", "BEGIN_DATE_TIME"]
    )
    ...
    if notable.empty:
        ...
        for _, area in notable.iterrows():
            print(
                f"  {area['eo_number']} {area['CZ_NAME']} "
    ...
    for (eo_number, description), group in result.groupby(
        ["eo_number", "event_description"], sort=False
    ):
        print(f"  {eo_number} ({description})")
