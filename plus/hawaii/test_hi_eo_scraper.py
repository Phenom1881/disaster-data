...
import hi_eo_scraper as scraper
import hawaii as wrapper
...
class ArchiveTests(unittest.TestCase):
    def test_live_archive_fixture(self):
        ...
        records = scraper.parse_archive(content, 'https://governor.hawaii.gov/category/newsroom/emergency-proclamations/')
    ...
    def test_classification_boundaries(self):
        record = make_record('HI', 'EXECUTIVE ORDER Extending Executive Order D 2020 168 Declaring that Conditions of Extreme Fire Hazard Exist', json.loads((BASE / 'fixtures/expected_record.json').read_text())['archive_record_url'], signed='2020-09-18')
    ...
    def test_wrapper_contract(self):
        with tempfile.TemporaryDirectory() as directory, patch.object(wrapper.subprocess, 'run') as run:
            ...
            self.assertEqual(Path(cmd[1]).name, 'hi_eo_scraper.py')
            self.assertEqual(cmd[2::2], ['--actions-out', '--relationships-out', '--join-out'])
            self.assertEqual(wrapper.STATE, 'HI')
