...
def render_page(j, others):
    ...
    rows = "".join("<tr><td>%s</td><td>%s</td><td><span class='tag' title='%s'>%s</span></td><td>%s</td><td>%s</td></tr>"
                   ...
                   for r in j["recent"])
    history = ('<div class="tablewrap"><table><thead><tr><th>Date</th><th>Number</th><th>Type</th>'
    ...
    copyjs = ("<script>document.addEventListener('click',function(ev){var b=ev.target.closest('.copybtn');"
              "if(!b)return;var rows=JSON.parse(b.getAttribute('data-hmp'));"
              ...
              "function(){window.prompt('Copy:',t);});}else{window.prompt('Copy:',t);}});</script>")
