...
# Duplicated from build.py's STATE_NAMES (kept identical) rather than
# imported: build.py runs live OpenFEMA fetches as top-level code, so
# importing it here to reach one dict would trigger network calls at import
# time. Used only for unnamed_cluster_label()'s location suffix.
STATE_NAMES = {
...
def unnamed_cluster_label(incident_types, begin_date, states=None):
    """Display name for a newly formed unnamed cluster (two or more unnamed
    declarations merged with no storm to name them), e.g.
    'Severe Storm (April 2011, TN)'. Falls back to a generic incident label
    if the merged declarations do not share one incident type.

    states: optional set/iterable of USPS state codes touched by the merged
    group. Two identically-worded clusters in the same month (e.g. two
    separate "Severe Storm (April 2011)" events in different regions) are
    otherwise indistinguishable in a list; naming the state(s) is what lets
    a reader tell them apart without opening each one. Omitted (None or
    empty) falls back to the old date-only label rather than guessing.
    """
    types = {t for t in incident_types if t}
    label = sorted(types)[0] if len(types) == 1 else "Severe Weather"
    when = begin_date.strftime("%B %Y")
    where = _location_suffix(states)
    if where:
        return "%s (%s, %s)" % (label, when, where)
    return "%s (%s)" % (label, when)


def _location_suffix(states):
    """A short, readable location tag for a set of USPS state codes, e.g.
    'Tennessee', 'Tennessee and Kentucky', or 'Tennessee and 3 other states'.
    Unknown codes (not in STATE_NAMES) are shown as-is rather than dropped,
    so a real state is never silently omitted from the label."""
    names = sorted(STATE_NAMES.get(s, s) for s in (states or ()) if s)
    if not names:
        return ""
    if len(names) == 1:
        return names[0]
    ...
    return "%s and %d other states" % (names[0], len(names) - 1)
